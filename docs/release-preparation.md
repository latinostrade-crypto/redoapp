...
Update 2026-09-06: пакет `512769a` уже отправлен и выложен на оба существующих
сервиса. Backup выполнен; команды, build filters и headers применены через
Dashboard, On Commit восстановлен. Фактические deploy IDs и результаты smoke
см. в `docs/live-release-audit.md`. Последующие пункты остаются runbook для
следующего релиза, а не перечнем ещё не выполненной текущей выкладки.
