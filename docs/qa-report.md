...
- межсценный handoff больше не семплирует соседний `ComicScene`: уходящий canvas
  только рассыпает собственную сцену, а входящий canvas собирает собственный
  первый кадр и copy, исключая двойное владение и сломанный дубликат;
...
занимают `0.16–0.26`, межсценное падение — `0.12`, owned entrance следующей
главы — `0.09` desktop / `0.105` mobile, scrub равен `0.62/0.9`. Пылинки обеих
фаз остаются у нижнего края stage, поэтому чёрного hold между ними нет.

Главный copy собирается particle-полем одновременно с первым изображением новой
главы только её собственным entrance и не повторяет dissolve при внутренних сменах. Bubbles,
