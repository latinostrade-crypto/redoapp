...
assert.equal(translateTableEvent('RARE DROP! +50 chips added to your balance.', translateGame), 'РЕДКАЯ НАГРАДА! Зачислено фишек: +50.');
...
const openNotice = message('reservePrivateJoin', { stake: 50, game: 'POKER' });
const renderNotice = () => renderUiMessage(openNotice, (id, values) => String(i18n.t(id, { ...values, ns: 'game' })));
assert.equal(renderNotice(), 'Зарезервировать 50 фишек для входа за приватный стол POKER?');
...
assert.equal(renderNotice(), 'Reserve 50 chips to join this private POKER table?');
